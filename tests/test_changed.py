...
def test_output_changed_function(get_changed_files_mock, testdir, config_mock):
    """
    In case of changes just to specific functions only their names will be detected.
    """
    diff = MagicMock()
...
@patch("pytest_changed.get_changed_files")
def test_output_changed_class_and_function(get_changed_files_mock, testdir, config_mock):
    """
    In case of changes inside the class body, the class names should be detected.

    To make sure all tests inside the class are running as changes in the class definition
    may affect test functions defined in the scope of the class.

    When pytest receives a class it will run all test-functions that are contained.
    """
    diff = MagicMock()
